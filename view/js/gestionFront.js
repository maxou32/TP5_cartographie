				...
				*/
				...
				function onPopupOpen(){
					...
					$(".point-delete-button:visible").click(function () {
						...
						$('#origin').text('front');						
						$('#question').html ('Confirmez-vous la suppression définitive du front de '+sessionStorage[cleid] +' ?');
			...
			document.getElementById("btnOui").addEventListener("click", function(e){
				...
				var cleid =cle+'/id';
				var clei=cle+'/i';
				alert('front supprime : id= '+sessionStorage[cleid]+" i= "+sessionStorage[clei]);
				if($('#origin').text()==="front"){
					supprimeFront(sessionStorage[cleid]);
					delete newdonnees[sessionStorage[clei]];
					mymap.removeLayer(tempPoint);
				}else{
					console.log("supprime ligne en base = "+sessionStorage['cleid']);
					supprimeLigneFront(sessionStorage[cleid]);
					delete newdonnees[sessionStorage[clei]];
